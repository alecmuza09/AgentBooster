...
export const leadStatusConfig: Record<LeadStatus, { label: string; icon: React.ElementType; colorClasses: string }> = {
  'No Contactado': { label: 'No Contactado', icon: Clock, colorClasses: 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300' },
  'Contactado': { label: 'Contactado', icon: Phone, colorClasses: 'bg-blue-100 text-blue-600 dark:bg-blue-900/50 dark:text-blue-400' },
  'Cita Agendada': { label: 'Cita Agendada', icon: CalendarClock, colorClasses: 'bg-purple-100 text-purple-600 dark:bg-purple-900/50 dark:text-purple-400' },
  'Propuesta Trabajada': { label: 'Propuesta Trabajada', icon: Mail, colorClasses: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/50 dark:text-yellow-400' },
...
export const LeadStatusBadge: React.FC<{ status: LeadStatus }> = ({ status }) => {
  const config = leadStatusConfig[status] || leadStatusConfig['No Contactado'];
